...
kwin_script_dir="$data/kwin/scripts/vibrance-focus"
gnome_ext_dir="$data/gnome-shell/extensions/vibrance@vibrance.github.io"
...
install_all() {
    ...
    mkdir -p "$bindir"
    install -m755 "$repo/target/release/vibrance" "$bindir/vibrance"
    ...
    if [ "$mode" = system ]; then
        ...
        sudo install -Dm755 "$so" "$qt_plugin_sys/vibrance.so"
    else
        say "Installing KWin effect to $qt_plugin_user"
        install -Dm755 "$so" "$qt_plugin_user/vibrance.so"
    ...
    cp -r "$repo/assets/gnome-extension/vibrance@vibrance.github.io/." "$gnome_ext_dir/"

    say "Installing systemd user unit"
    mkdir -p "$unit_dir"
    install -m644 "$repo/packaging/vibrance-daemon.service" "$unit_dir/vibrance-daemon.service"

    say "Installing desktop entry"
    install -Dm644 "$repo/packaging/vibrance.desktop" "$data/applications/vibrance.desktop"

    cat <<EOF

Installed. Next steps:
  * Effect:   vibrance on 1.5      (load + boost)   /   vibrance off
  * Per-game: vibrance run --profile cs2 -- %command%   (Steam launch option)
